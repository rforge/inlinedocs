...
checked for validity using \inlinedocs\ version 1.9. For brevity, only
...
The \inlinedocs\ package detects which functions are S3
...
\code{package.skeleton.dx()}, which generates Rd files for a
package. For example, \code{package.skeleton.dx("fermat")} processes \R\
code found in \code{fermat/R}, and generates Rd files in
\code{fermat/man}.
The generated Rd files
should be treated as object files, since any edits will be overwritten
the next time the Rd files are generated.
...
Currently, \inlinedocs\ is incapable of generating Rd files that
...
<<>>=
my.parsers <- list(forfun(title.from.firstline),forfun(simple))
@ 
...
<<echo=FALSE>>=
docs <- extract.docs.file("alternate.R",my.parsers)
str(docs)
@
...
such as \code{parse\_Rd()}, as described by \cite{parseRd}. For
package authors who want to convert Rd files to inlinedocs comments,
we may be able to use \code{parse\_Rd()} to develop a converter that
takes .R source code and .Rd documentation, then outputs .R code with
inlinedocs.
