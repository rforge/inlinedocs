...
The \code{package.skeleton()} function
that ships with base \R\ is intended to ease the generation of Rd
files from \R\ code \citep{R}. After specifying some input \R\ code
files or objects to use for the package, it produces some bare-bones
documentation that you must fill in using a text editor. Although
\code{package.skeleton()} is sufficient for creating small packages
that are published once and forgotten, it offers little help for packages
