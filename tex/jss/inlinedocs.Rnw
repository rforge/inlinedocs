...
spaces, brackets, default arguments and commas is quite flexible.
<<>>=
...
\subsection{Inline titles, arguments, other sections, and list return values}
...
(n,    ##<< Integer to test for pseudoprimality.
 times ##<< Number of Fermat tests to perform. More tests are more 
  ...
  ##value<<
  list(prime=result, ##<< logical TRUE if probably prime.
       n=n,          ##<< integer tested.
       times=times)  ##<< number of tests performed.
  ##end<<
}
@
...
of lists using the list names defined in the code. The list entries
are documented using \verb+##<<+ in the same way as function
