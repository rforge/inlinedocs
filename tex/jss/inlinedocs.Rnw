...
this, and we first review these alternate implementation, emphasing on the
key aspects that justify the introduction of a new implementation like
\inlinedocs.
...
format, also included in the default installation of \R allows integration of
R code and results within \LaTeX\ documents \citep{latex}. However, the goal
...
maintain. These packages are a step in the direction of a better integration
of the code and its documentation, but they have three major drawbacks:
...
\item They only use comments to generate documentation, ignoring the
  ...
  documentation (and is source of incoherence if both are not
...
\item The documentation for an object appears in comments above its
  ...
  thus, they grow far away from the actual documented code.
\item Examples are defined either in comments or in supplementary R
  ...
  the R interpreter, and supplementary R code files reintroduce the separation
  of code and documentation that those tools are supposed to Žliminate.
\end{enumerate}
...
in the \code{ex} attribute of the object:
<<>>=
...
%% PhG: may be is it worth to provide an alternate way to include examples
%% that apporaches allows to update examples without updating the function
%% definition

Examples can also be added in a second step like here:
<<>>=
add2 <- function (x) {
  x + 2
}

inlinedocExample(add2) <- function () {
  add2(11) == 13
}
@
...
%% PhG: I wonder if a runExample() function would not be nice to add in the
%% inlinedocs package at this stage? So, examples could be checked *before*
%% compiling the package in development
...
The inheritance referred to in that example is designed to avoid the
need for repetitive documentation when defining a family of methods in
a class hierarchy. The argument descriptions and other documentation
sections default to those defined in the parent class. At present it
only functions when all the definitions are within a single source
file and this ``documentation inheritance'' is strictly linear within
the file.
...
\subsection{Editor support and workflow suggestions}
\label{sec:ess}
