...
In ESS 5.10, the \verb+ess-function-pattern+ regular expression does not
allow comments and/or newline between the keyword \verb+function+ and the
parenthesis which opens the argument list. That may be remedied by
replacing the trailing \verb+function\\s-*(+ with
\verb+function\\s-*\\(?:\\(?:#.*\\)?\n\\s-*\\)*(+.
