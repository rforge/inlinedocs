%  -*- compile-command: "make"; compilation-read-command: nil -*- 
...
In all of them the \R-code is plain text and instead of part of a comment.

The following example demonstrates attaching example code to the \code{center} functin as an attribute.

<<>>=
attr(center,"ex") <- function(){
  x <- runif(10)
  c( mean=center(x), median=center(x,method="median") )
}
@

Other ways of attaching example code are, code after the return statement of
a function, or code in a file \code{test/<Functionname>.R}.
