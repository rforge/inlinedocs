...
\begin{document}
<<echo=FALSE>>=
.libPaths("/nas/home1/h/hocking/lib64/R/library")
library(inlinedocs)
...
\code{package.skeleton.dx("path/to/your/pkgdir")}, which generates Rd
...
\inlinedocs\ this should be sufficient for everyday use. For users who
wish to extend the syntax of \inlinedocs, the following sections
explain the internal organization of the \inlinedocs\ package.



\subsection{Documentation Lists}

The Documentation List is central to \inlinedocs. It is a list of
lists that describes all of the documentation to write to the Rd
files. The elements of the outer list correspond to Rd files in the
package, and the elements of the inner list correspond to tags in the
Rd file. For example, consider the following function and its
corresponding documentation list.
...
inner.list <- prefixed.lines(attr(give.me.five,"source"))
str(inner.list)
@ 

This list describes the tags in the Rd file that will be written for
\code{test}. Furt
