...
code. Furthermore, \inlinedocs\ allows you to write documentation
wherever it is most relevant in the code using \verb+##section<<+
comments. These special comment strings are designed to work well with
...
\Roxygen\ vignette \citep{roxygen}.

\subsection{Documenting function signatures and return values}

The following example demonstrates the minimal documentation you
...
\item For each argument, an item is created in the \code{arguments}
  section using the immediately following \verb+###+ comments.
...
set to the function name, so with this minimal level of documentation
you should be able to get a working package that passes \code{R CMD
...
(n,    ##<< Integer to test for pseudoprimality.
 times ##<< Number of Fermat tests to perform. More tests are more 
...
On the first line, the \verb+#+ comment specifies the title.
On an argument line, a \verb+##<<+ comment and any immediately
following \verb+##+ lines specify its documentation.
In each comment, arbitrary Rd may be written, as shown in the
\verb+##seealso<<+ section above.

A \verb+##section<<+ comment can be used for any documentation section
except for \code{examples}, which is handled in a special manner as
shown below in section~\ref{sec:exattr}. Each \verb+##section<<+ may
occur several times in a single documentation object. Each occurance
is normally concatenated as separate paragraphs, but special
processing is applied to match the intended use of the following:
\begin{description}
...
On the final line, the \verb+##value<<+ comment allows documentation
of lists or data frames using the names defined in the code. The
entries are documented using \verb+##<<+ in the same way as function
arguments, and this construct can even be nested.

\label{sec:exattr}

The examples are written using \code{structure()} to put them in the
\code{ex} attribute. This method for documenting examples was
motivated by the desire to express examples in \R\ code rather than in
\R\ comments, and to keep the examples close to the object
definition. When examples are in \R\ code, they are easily transferred
to the \R\ interpreter, and thus are easy to debug. Furthermore, when
...
syntax and generating Rd files. It has been in development since
