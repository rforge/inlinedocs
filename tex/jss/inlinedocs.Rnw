...
it is possible to mix plain .Rd files and inlinedocs in the same
package. By default inlinedocs will overwrite .Rd files for all
objects in the package, but you can specify the static Rd files that
...
It is currently impossible to generate Rd files that document multiple
objects using inlinedocs, but package authors may write these Rd files
manually using this mechanism.
