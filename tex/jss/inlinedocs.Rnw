...
which is then stored in a Documentation List, as explained in the
following sections.
...
the \code{give.me.five} function. The \code{prefixed.lines} Parser
Function examines the character vector of lines of code that define
the function, and returns a list of documentation tags.  

The mechanism of \inlinedocs\ is to run several such Parser Functions
on each item in the package, then combine their output to produce a
Documentation List that describes all the items in the package.
