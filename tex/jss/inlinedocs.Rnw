...
code files, for documentation generation using markup in R comments.
...
\item The documentation for an object appears in comments above its
  definition. These comment blocks can get quite large, and thus far
  away from the actual code documented.
\item Examples are defined either in comments or in supplementary R
  code files. Examples in comments are not easy to test with the R
  interpreter, and supplementary R code files are far away from the
  actual code documented.
\end{enumerate}
...
But how do this Parser Function get access to the \code{src} argument
