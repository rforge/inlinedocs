...
% [??] Issue1, from reviewer 2: how to incorporate into R toolchain?
...
% should not be edited. Edit: emphasize that package.skeleton.dx()
...
% be mixed?  Response: currently inlinedocs will only generate .Rd for
...
% [??] Issue3 Can inlinedocs generate an Rd file that documents multiple
% objects? Response: no, but you can write these .Rd files by hand if
% you like. Edit: add to the section "package.skeleton.dx() for
% generating Rd files."

% [??] Issue4 from reviewer 1 comments 4 and 6: debugging code with
...
% [TW?] Issue5 from reviewer 2: need to compare with tools from other
...
% [??] Issue7 from reviewer 2: why not sweave? Response: because we need a
% parser and sweave is not the best tool for this. Edit: add 1
% sentence in the introduction.

% [??] Issue8 from reviewer 1 comment 5: can no longer use "source"
