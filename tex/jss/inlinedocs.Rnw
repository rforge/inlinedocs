...
different. It aims for integration of documentation inside of \R\ code
files, to generate Rd documentation files using \R\ code and markup in
\R\ comments. Thus for \inlinedocs\ we need to extract the
documentation specified in \R\ code, and the Sweave format and function
can not be easily applied to this parsing task.
...
be completed using a text editor. Although \code{package.skeleton()}
...
\citep{wikipedia}. These systems use large comments in headers,
whereas \inlinedocs\ uses smaller comments alongside the code, and
exploits the code structure to reduce the need to repeat information
