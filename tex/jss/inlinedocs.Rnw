...
\author{Toby Dylan Hocking\\INRIA Paris \And 
        ...
        Keith Ponting\\Aurix Ltd., UK}
\title{Sustainable, extensible documentation generation using \inlinedocs}

%% for pretty printing and a nice hypersummary also set:
\Plainauthor{Toby Dylan Hocking, Thomas Wutzler, Keith Ponting} %% comma-separated
...
this, and we first review these efforts.
...
documents \citep{latex}. However, the goal of \inlinedocs\ is
different. It aims for integration of Rd documentation inside of R
code files, for documentation generation using R code and markup in R
comments.

For this purpose, there are already several different approaches,
including the \code{package.skeleton} function which ships with base
\R\ \citep{R}. After specifying some input R code files or objects to
use for the package, it produces some bare-bones documentation that
...
closer to the actual code it documents, which is easier to
maintain. These packages are a step in the right direction, but they
have 3 key drawbacks:
...
\item The documentation for an object appears in comments above its
  ...
  thus far away from the actual code documented.
\item Examples are defined either in comments or in supplementary R
  ...
  the R interpreter, and supplementary R code files are far away from
  the actual code documented.
\end{enumerate}

\subsection{Documentation using inline comments}

The \inlinedocs\ package addresses these issues by proposing a new
syntax for inline documentation of \R\ packages. Using \inlinedocs,
...
resulting code is free of duplication and incredibly easy to maintain.
...
\label{sec:exattr}
The following code demonstrates the preferred method of documenting
examples. It uses the \code{structure} function to define the example
in the \code{ex} attribute of the object, thus avoiding repetition of
the object name:
<<>>=
...
they are easily transferred to the R interpreter, and thus are easy to
debug. Furthermore, when examples are written close to the object
definition, it is easier to keep examples up to date and informative.
...
The \inlinedocs\ package provides provisional support for documenting S4
class declarations which use the \verb+setClass+ function or for S3 class
objects generated by the \verb+R.oo+ package \citep{Roo} using the
\verb+setConstructorS3+ or \verb+setMethodS3+.\footnote{S4 generic methods
  have yet to be supported.} The following example is from the source of
\inlinedocs:
<<>>=
...
The inheritance referred to in that example is designed to avoid the need
for repetition when defining a family of methods for a class hierarchy -
argument descriptions and other documentation sections default to those
from a previous definition. At present it only functions when all the
definitions are within a single source file and this ``documentation
inheritance'' is strictly linear within the file.

S3 generic functions are automatically detected and the appropriate
\verb+\method+ markup inserted in the documentation.
...
\subsection{Relationship with ESS}
\label{sec:ess}
...
In ESS 5.10, an entire function may be sent for evaluation using
\verb+ess-eval-function+ command. However the standard
\verb+ess-function-pattern+ regular expression does not allow comments
and/or newline between the keyword \verb+function+ and the parenthesis
which opens the argument list. That may be remedied by replacing the
trailing \verb+function\\s-*(+ with
\verb+function\\b\\s-*\\(?:\\(?:#.*\\)?\n\\s-*\\)*(?+.

Additional changes
(documented on the web site) are required to
that pattern and to \verb+ess-set-function-start+ are required to transmit
a ``function with examples'' structure as described in section
\ref{sec:exattr}. 
...
and \code{docs} arguments, and call the object-specfic Parser Function
...
Some work is still required to fully support S4 classes and the new
reference classes.
