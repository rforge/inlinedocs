...
For documentation generation, there are already several different
approaches, including the \code{package.skeleton} function which ships
with base \R\ \citep{R}. After specifying some input R code files or
objects to use for the package, it produces some bare-bones
documentation that you must fill in using a text editor.
