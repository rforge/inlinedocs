...
\begin{frame}[containsverbatim]
  ...
  \\
  generates \texttt{softThresh/man/soft.threshold.Rd}
\end{frame}
...
\framet{inlinedocs for documentation generation}{
  ...
  \item Almost eliminates the possibility of code and documentation
    conflicts.
  \item AND: the syntax is easily extensible, there is support for S4
    methods.
  }
...
\framet{To publish your package}{
  ...
  \item \texttt{R CMD check pkgdir}
  \item \texttt{R CMD build pkgdir} (makes \texttt{pkgdir.tar.gz})
  \item Upload to ftp://cran.r-project.org/incoming 
    \begin{itemize}
    \item     user: anonymous
