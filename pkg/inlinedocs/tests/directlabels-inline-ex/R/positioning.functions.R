...
### only a single group, and returns the direct label position.
  ...
  return(structure(function(d,...)ddply(d,.(groups),f,...),"source"=src))
...
### Arguments to pass to transform.
  ...
  return(function(d,...)do.call("transform",c(list(d),L)))
### A Positioning Function.
  complicated <- list(dl.trans(x=x+10),
                    dl.indep(d[-2,]),
                    rot=c(30,180))
...
### The plot object, with direct labels added.
  ...
  direct.label(sprayplot,last.points)
  ...
  print(direct.label(update(p,xlim=c(0,9.25)),
                     list(last.points,cex=0.75,dl.trans(x=x+0.1))))
}
lines2 <- function
